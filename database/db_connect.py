...
def create_connection(host_name, user_name, user_password, db_name):
    ...
    try:
        ...
        cursor.execute('CREATE TABLE IF NOT EXISTS tour(img TEXT, name VARCHAR(24) PRIMARY KEY, description TEXT, '
...
async def db_add_tour(tour):
    cursor.execute('INSERT INTO tour (img, name, description, rating, from_city, to_city, when_date, days, price)'
...
async def db_get_tour(message: types.Message):
    cursor.execute('select img, name, description, price from tour')
...
async def db_delete_tour(data):
    cursor.execute('delete from tour where name = %s', (data, ))
    connection.commit()
